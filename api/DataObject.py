...
class DataObject(list):
    ...
    def where_condition(self, column: DDD) -> str:
        """Return formatting for condition column based on datatype. This function recognizes two conversions: TIMESTAMP converts the column value into integer and DATETIME is converted into ISO8601 datetime string."""
        col = None
        for c in self:
            if c.name == column:
                col = c
        ...
        if col.datatype == 'TIMESTAMP':
            return f"CAST(strftime('%s', {col.name}) as integer)"
        elif col.datatype == 'DATETIME':
            return f"datetime({col.name})"
        else:
            return f"{col.name}"


    def schema(self, exclude: list = []) -> dict:
        """Table's data schema dictionary. Caller needs to set 'readonly' values as necessary, before converting to JSON and sending the schema out. Schema is required by certain libraries that automatically generate forms from JSON."""
        #
...
if __name__ == '__main__':
    ...
    with sqlite3.connect("../application.sqlite3", isolation_level = None) as db:
        cursor = db.cursor()
        teacher = DataObject(cursor, 'teacher')
        print(teacher.get_column_names())
        print(teacher.get_column_objects())

        # Schema
        print("Schema for the table:")
        print(teacher.schema())

        print("Insert SQL:")
        print(teacher.insertSQL({'uid': 'jasata'}))

        print("Update SQL:")
        print(teacher.updateSQL({'uid': 'jasata', 'ses': 2}, ['uid']))

        print("Select SQL:")
        print(teacher.selectSQL({'uid': 'jasata', 'ses': 2}))
